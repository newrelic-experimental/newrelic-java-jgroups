		...
		if(evt.getType() == Event.MSG) {
			...
			if (Utils.enabled) {
				...
				if (header != null && header instanceof NRHeader) {
					...
					MessageConsumeParameters params = MessageConsumeParameters.library("JGroups")
							.destinationType(DestinationType.EXCHANGE).destinationName(destName).inboundHeaders(wrapper)
							.build();
					NewRelic.getAgent().getTracedMethod().reportAsExternal(params);
				} 
			}
