...
public abstract class RequestCorrelator {
	...
	@Trace(dispatcher=true)
	public void sendRequest(long id, Collection<Address> dest_mbrs, Message msg, RspCollector coll, RequestOptions options) {
		NewRelic.getAgent().getTracedMethod().setMetricName(new String[] {"Custom","RequestCorrelator","sendRequest"});
		Weaver.callOriginal();
	}
	
	@Trace(dispatcher=true)
	public void sendUnicastRequest(long id, Address target, Message msg, RspCollector coll) {
